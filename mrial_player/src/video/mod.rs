...
use ffmpeg_next::{format::Pixel, frame, software};
...
impl VideoThread {
    ...
    fn rgb_to_slint_pixel_buffer(rgb: &[u8]) -> slint::SharedPixelBuffer<slint::Rgb8Pixel> {
        let mut pixel_buffer = slint::SharedPixelBuffer::<slint::Rgb8Pixel>::new(
            TARGET_WIDTH as u32,
    ...
    pub fn begin_decoding(
        ...
        let _video_thread = thread::spawn(move || {
            let mut _simple_scalar =
                software::converter((W as u32, H as u32), Pixel::YUVJ422P, Pixel::RGB24).unwrap();
